...
export default async function update(options, host) {
  if (!options.force) {
    const lastCheck = await host.store.get('selfVersion');
    if (lastCheck && host.version === lastCheck.version && Date.now() - Date.parse(lastCheck.time) < 24 * 60 * 60 * 1000) {
  ...
  try {
    ...
    if (options.shouldInstall?.(release.version, url, marked(`# ${release.title}\n\n${release.notes}`).trimEnd())) {
