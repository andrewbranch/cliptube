...
import { success, fail, getYouTubeIdFromURL } from './utils.js';
 ...
 */
export default async function download({ url, outDir, overwrite }, host, handlers = {}) {
  const { fs, net } = host;
  ...
  if (formats.length === 2) {
    ...
    const [onVideoProgress, onAudioProgress] = handlers?.onDownloadProgress ? splitProgressHandler(handlers.onDownloadProgress) : [];
    try {
  ...
  const result = await downloadFormat(info, formats[0], outFile, host, handlers?.onDownloadProgress);
 ...
 * @param {string} outDir
 */
export function getOutFilePath(url, outDir) {
  const id = getYouTubeIdFromURL(url);
...
function mergeStreams(videoPath, audioPath, outPath, durationMs, { ffmpeg }, handlers) {
  ...
  return new Promise((resolve, reject) => {
    cmd.on('update', (/** @type {any} */ data) => handlers?.onMergeProgress?.(data.out_time_ms));
 ...
 */
export function createInteractiveDownloadHandlers(host) {
  const downloadBar = new progress.SingleBar({ fps: 1 }, progress.Presets.legacy);
  const mergeBar = new progress.SingleBar({ fps: 0.5 }, progress.Presets.legacy);
  return {
    onInfo: info => {
      host.log();
